...
export default function RoleBadge({ role, size = 'sm', fullWidth = false }) {
  ...
  return (
    <span className={`inline-flex items-center justify-center font-semibold rounded-full border ${config.color} ${sizeClasses[size]} ${fullWidth ? 'w-full' : ''}`}>
